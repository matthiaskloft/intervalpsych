...
test_that("ilr transformation handles invalid input", {
  ...
  expect_error(ilr(data.frame(rbind(c(0.1, 0.2), c(0.4, 0.5)))), "Simplex must have 3 elements")
})
...
test_that("inv_ilr transformation handles invalid input", {
  expect_error(inv_ilr(c(0.4)), "Error: \\(row-\\)vector must have exactly 2 elements\\!")
  expect_error(inv_ilr(data.frame(rbind(c(0.1), c(0.4)))), "Error: \\(row-\\)vector must have exactly 2 elements\\!")
})
...
test_that("itvl_to_splx transformation works for dataframe input with 3 elements per row", {
  interval_bounds <- data.frame(rbind(c(0.1, 0.3, 0.6), c(0.2, 0.4, 0.7)))
  result <- itvl_to_splx(interval_bounds, min = 0, max = 1)
...
test_that("itvl_to_splx transformation handles invalid input", {
  expect_error(itvl_to_splx(c(0.4), min = 0, max = 1), "Raw data must have either 2 or 3 values per response\\!")
  expect_error(itvl_to_splx(data.frame(rbind(c(0.1), c(0.4))), min = 0, max = 1), "Raw data must have either 2 or 3 values per response\\!")
})
...
# Test for vector input with 4 elements
test_that("splx_to_itvl transformation works for vector input with 4 elements", {
  simplex <- c(0.1, 0.3, 0.4, 0.2)
  result <- splx_to_itvl(simplex, min = 0, max = 1)
  expect_equal(length(result), 3)
  expect_named(result, c("x_lo", "x_mid", "x_up"))
})
...
# Test for dataframe input with 4 elements per row
test_that("splx_to_itvl transformation works for dataframe input with 4 elements per row", {
  simplex <- data.frame(rbind(c(0.1, 0.3, 0.4, 0.2), c(0.2, 0.2, 0.3, 0.3)))
  result <- splx_to_itvl(simplex, min = 0, max = 1)
  expect_equal(ncol(result), 3)
  expect_equal(nrow(result), 2)
  expect_named(result, c("x_lo", "x_mid", "x_up"))
...
test_that("splx_to_itvl transformation handles invalid input", {
  expect_error(splx_to_itvl(c(0.4, 0.2), min = 0, max = 1), "Simplex must have either 3 or 4 elements")
  expect_error(splx_to_itvl(data.frame(rbind(c(0.1, 0.2), c(0.4, 0.5))), min = 0, max = 1), "Simplex must have either 3 or 4 elements")
})
